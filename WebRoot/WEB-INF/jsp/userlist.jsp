        ...
        <div class="right">
            ...
            <div class="search">
           		<form method="get" action="query">
					 <input type="hidden" name="pageNo" id="pageNo" value="${helper.pageNo}<c:if test="${helper.pageNo eq null}">1</c:if>"/>
					 <input type="hidden" name="pageSize" value="${helper.pageSize}<c:if test="${helper.pageSize eq null}">5</c:if>"/>
					 <span>用户名：</span>
					 <input name="params['userName']" class="input-text" type="text" value="">
					 <span>用户角色：</span>
					 <select name="params['userRole']">
            ...
            <table class="providerTable" cellpadding="0" cellspacing="0">
                <tr class="firstTr">
                   ...
                   <c:forEach var="user" items="${helper.results}" varStatus="status">
					<tr>
						<td>
